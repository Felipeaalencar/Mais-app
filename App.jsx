...
function ConsultorClinico() {
  ...
  const SUGESTOES = ['Dose maxima de noradrenalina?', 'Criterios de Berlin SDRA', 'Quando intubar Glasgow 12?', 'Protocolo sepse 1 hora'];
  ...
  async function send(text) {
    ...
    try {
      const res = await fetch('/api/analyze', {
        ...
        body: JSON.stringify({ caseText: q, prompt: 'Voce e um consultor medico especialista em UTI. Responda duvidas clinicas de forma direta e baseada em evidencias (Surviving Sepsis, SCCM, UpToDate). Seja conciso. Mencione doses e criterios quando relevante. Responda em portugues.' })
      });
      const data = await res.json();
      const raw = (data.content || []).map(b => b.type === 'text' ? b.text : '').join('');
      setMsgs(prev => [...prev, { role: 'ai', text: raw || 'Nao consegui responder.' }]);
    } catch { setMsgs(prev => [...prev, { role: 'ai', text: 'Erro de conexao.' }]); }
    setLoading(false);
  }

  return (
    <>
      <button onClick={() => setOpen(!open)} title="Consultor Clinico" style={{ position: 'fixed', bottom: '24px', right: '24px', width: '54px', height: '54px', borderRadius: '50%', background: 'linear-gradient(135deg,#1D9E75,#0d7a5a)', border: 'none', cursor: 'pointer', fontSize: '22px', zIndex: 1000, boxShadow: '0 4px 16px #1D9E7540' }}>
        {open ? '✕' : '💬'}
      </button>
      {open && (
        <div style={{ position: 'fixed', bottom: '90px', right: '24px', width: '340px', height: '460px', background: '#0a0f1e', border: '1px solid #1e293b', borderRadius: '14px', display: 'flex', flexDirection: 'column', zIndex: 1000, overflow: 'hidden' }}>
          <div style={{ padding: '12px 16px', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '11px', fontWeight: 700, color: '#1D9E75', letterSpacing: '0.1em' }}>CONSULTOR CLINICO</span>
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {msgs.length === 0 && SUGESTOES.map((s, i) => (
              <div key={i} onClick={() => send(s)} style={{ fontSize: '11px', color: '#64748b', background: '#0f172a', border: '1px solid #1e293b', borderRadius: '20px', padding: '7px 14px', cursor: 'pointer', textAlign: 'center' }}>{s}</div>
            ))}
            {msgs.map((m, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                <div style={{ maxWidth: '85%', padding: '9px 12px', borderRadius: m.role === 'user' ? '10px 10px 2px 10px' : '10px 10px 10px 2px', background: m.role === 'user' ? '#1e3a5f' : '#0f172a', border: '1px solid ' + (m.role === 'user' ? '#1e4a7f' : '#1e293b') }}>
                  {m.role === 'ai' && <div style={{ fontSize: '8px', color: '#1D9E75', marginBottom: '4px' }}>MAIS</div>}
                  <p style={{ fontSize: '12px', color: m.role === 'user' ? '#93c5fd' : '#94a3b8', lineHeight: 1.6, margin: 0, whiteSpace: 'pre-wrap' }}>{m.text}</p>
                </div>
              </div>
            ))}
            {loading && <div style={{ padding: '9px 12px', background: '#0f172a', border: '1px solid #1e293b', borderRadius: '10px', width: 'fit-content' }}><span style={{ fontSize: '12px', color: '#475569' }}>Consultando...</span></div>}
            <div ref={endRef} />
          </div>
          <div style={{ padding: '10px', borderTop: '1px solid #1e293b', display: 'flex', gap: '8px' }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder="Digite sua duvida..." className="inp" style={{ flex: 1, fontSize: '12px' }} />
            <button onClick={() => send()} disabled={!input.trim() || loading} className="btn" style={{ padding: '8px 14px', opacity: !input.trim() || loading ? 0.4 : 1 }}>→</button>
          </div>
        </div>
      )}
    </>
...
export default function App() {
  ...
  async function analyze() {
    ...
    try {
      ...
      if (!parsed) throw new Error('Resposta invalida');
      setResult(parsed);
      if (sel) {
  ...
  function analyzePhoto(file) {
    ...
    reader.onload = async e => {
      try {
        ...
        const raw = (data.content || []).map(b => b.type === 'text' ? b.text : '').join('');
        const parsed = fixJSON(raw);
        setCaseText(parsed?.resumo ? 'EXAMES DA FOTO:\n' + parsed.resumo + '\n\nAdicione informacoes clinicas e clique em Analisar caso.' : 'Nao consegui ler. Digite manualmente.');
      } catch { setCaseText('Erro ao processar foto.'); }
    };
    reader.readAsDataURL(file);
  }

  function goHome() { setSel(null); setScreen('home'); setResult(null); setSelA(null); setEditPat(false); setNote(''); }
  ...
  const maisGrave = criticos[0] || atencao[0] || null;
  ...
  const homeFiltrados = homeFilter === 'critico' ? criticos : homeFilter === 'atencao' ? atencao : homeFilter === 'estavel' ? estaveis : ativos;
    ...
    <div style={{ display: 'flex', height: '100vh', background: '#020617', overflow: 'hidden' }}>
      ...
      <div style={{ width: '220px', background: '#0a0f1e', borderRight: '1px solid #1e293b', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '14px 12px', borderBottom: '1px solid #1e293b' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', cursor: 'pointer' }} onClick={goHome}>
            <Logo size={32} />
            <div>
              <div style={{ fontWeight: 700, fontSize: '16px', color: '#1D9E75', letterSpacing: '5px', lineHeight: 1 }}>MAIS</div>
              <div style={{ fontSize: '8px', color: '#475569', letterSpacing: '0.18em', marginTop: '2px' }}>inteligencia clinica.</div>
            </div>
          </div>
        ...
        {showNP && (
          <div style={{ padding: '12px', borderBottom: '1px solid #1e293b' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              <button className="btn" style={{ flex: 1 }} onClick={createP}>Salvar</button>
              <button className="sbtn" onClick={() => setShowNP(false)}>X</button>
            </div>
          </div>
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '8px' }}>
          ...
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} onClick={() => { setSel(p); loadA(p.id); setScreen('patient'); setResult(null); setSelA(null); setEditPat(false); setNote(''); }}
                style={{ padding: '8px', borderRadius: '7px', marginBottom: '4px', cursor: 'pointer', background: isSel ? '#1e3a5f' : 'none', border: '1px solid ' + (isSel ? '#1e4a7f' : 'transparent') }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2px' }}>
                  <span style={{ fontWeight: 600, fontSize: '12px', color: '#f1f5f9' }}>{p.nome}</span>
                  {p.desfecho
                    ? <span style={{ fontSize: '8px', fontWeight: 700, color: DESFECHO_CFG[p.desfecho]?.color, background: DESFECHO_CFG[p.desfecho]?.color + '20', borderRadius: '3px', padding: '1px 5px' }}>{DESFECHO_CFG[p.desfecho]?.label}</span>
                    : <span style={{ fontSize: '8px', fontWeight: 700, color: STATUS_CFG[p.status_clinico || 'estavel']?.color, background: STATUS_CFG[p.status_clinico || 'estavel']?.bg, borderRadius: '3px', padding: '1px 5px' }}>{STATUS_CFG[p.status_clinico || 'estavel']?.label}</span>
                  }
                </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
            ...
            {patients.length === 0 ? (
              ...
              <div>
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#f1f5f9' }}>Visao Geral — UTI</div>
                  <div style={{ fontSize: '10px', color: '#475569', marginTop: '2px' }}>{ativos.length} paciente{ativos.length !== 1 ? 's' : ''} ativo{ativos.length !== 1 ? 's' : ''}</div>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '10px', marginBottom: '16px' }}>
                  {homeMetrics.map(m => (
                    <div key={m.label} onClick={() => setHomeFilter(homeFilter === m.f ? null : m.f)}
                      style={{ background: homeFilter === m.f ? '#1e293b' : '#0f172a', border: '1px solid ' + (homeFilter === m.f ? m.color + '50' : '#1e293b'), borderRadius: '10px', padding: '14px', cursor: 'pointer' }}>
                      <div style={{ fontSize: '28px', fontWeight: 700, color: m.color, lineHeight: 1 }}>{m.val}</div>
                      <div style={{ fontSize: '9px', color: '#475569', marginTop: '5px', letterSpacing: '0.08em' }}>{m.label}</div>
                    </div>
                  ))}
                </div>

                {!homeFilter && maisGrave && (
                  <div onClick={() => { setSel(maisGrave); loadA(maisGrave.id); setScreen('patient'); }}
                    style={{ background: '#0f172a', border: '1px solid #ef444430', borderLeft: '3px solid #ef4444', borderRadius: '10px', padding: '14px', marginBottom: '16px', cursor: 'pointer' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontSize: '14px', fontWeight: 700, color: '#f1f5f9' }}>{maisGrave.nome}</div>
                        <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '3px' }}>{hipMG ? hipMG.nome : 'Ver analise'}</div>
                      </div>
                      {qMG !== null && <div style={{ textAlign: 'center' }}><div style={{ fontSize: '22px', fontWeight: 700, color: '#ef4444' }}>{qMG}/3</div><div style={{ fontSize: '8px', color: '#64748b' }}>qSOFA</div></div>}
                    </div>
                  </div>
                )}
                ...
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '10px', padding: '14px' }}>
                  ...
                  {homeFiltrados.map(p => {
                    const pa = (analyses[p.id] || [])[0];
                    const hip = pa?.resultado_json?.hipoteses?.[0];
                    return (
                      <div key={p.id} onClick={() => { setSel(p); loadA(p.id); setScreen('patient'); setHomeFilter(null); }}
                        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid #1e293b', cursor: 'pointer' }}>
                        <div>
                          <span style={{ fontSize: '13px', fontWeight: 700, color: '#f1f5f9' }}>{p.nome}</span>
                          <span style={{ fontSize: '10px', color: '#64748b', marginLeft: '8px' }}>{[p.leito, p.setor].filter(Boolean).join(' - ') || 'Sem setor'}</span>
                          {hip && <div style={{ fontSize: '10px', color: '#475569', marginTop: '2px' }}>{hip.nome}</div>}
                        </div>
                        <span style={{ fontSize: '9px', fontWeight: 700, padding: '2px 8px', borderRadius: '4px', color: STATUS_CFG[p.status_clinico || 'estavel']?.color, background: STATUS_CFG[p.status_clinico || 'estavel']?.bg }}>{STATUS_CFG[p.status_clinico || 'estavel']?.label}</span>
                      </div>
                    );
                  })}
                  {homeFiltrados.length === 0 && <div style={{ fontSize: '11px', color: '#334155', textAlign: 'center', padding: '12px' }}>Nenhum paciente nesta categoria</div>}
                </div>
              </div>
            )}
          </div>
        ...
        {/* PATIENT SCREEN */}
        {screen === 'patient' && sel && (
          <div className="fin" style={{ padding: '16px' }}>
            ...
            {showStatusEdit && (
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowStatusEdit(false)}>
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '12px', padding: '20px', width: '260px' }} onClick={e => e.stopPropagation()}>
                  ...
                  {Object.entries(STATUS_CFG).map(([k, v]) => (
                    <div key={k} onClick={async () => { await db.from('pacientes').update({ status_clinico: k }).eq('id', sel.id); setSel(p => ({ ...p, status_clinico: k })); setPatients(prev => prev.map(p => p.id === sel.id ? { ...p, status_clinico: k } : p)); setShowStatusEdit(false); }}
                      style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', borderRadius: '8px', cursor: 'pointer', background: sel.status_clinico === k ? v.bg : 'transparent', border: '1px solid ' + (sel.status_clinico === k ? v.border : 'transparent'), marginBottom: '6px' }}>
                      <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: v.color, flexShrink: 0 }} />
                      <span style={{ color: v.color, fontWeight: 700, fontSize: '13px' }}>{v.label}</span>
                    </div>
                  ))}
            ...
            {showDesfecho && (
              <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setShowDesfecho(false)}>
                <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '12px', padding: '20px', width: '260px' }} onClick={e => e.stopPropagation()}>
                  ...
                  {Object.entries(DESFECHO_CFG).map(([k, v]) => (
                    <div key={k} onClick={async () => { const now = new Date().toISOString(); await db.from('pacientes').update({ desfecho: k, desfecho_data: now }).eq('id', sel.id); setSel(p => ({ ...p, desfecho: k })); setPatients(prev => prev.map(p => p.id === sel.id ? { ...p, desfecho: k } : p)); setShowDesfecho(false); }}
                      style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', borderRadius: '8px', cursor: 'pointer', border: '1px solid ' + v.color + '30', marginBottom: '8px', background: v.color + '10' }}>
                      <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: v.color, flexShrink: 0 }} />
                      <span style={{ color: v.color, fontWeight: 700, fontSize: '14px' }}>{v.label}</span>
                    </div>
                  ))}
            ...
            {patA.map((a, i) => (
              <div key={a.id} onClick={() => { setSelA(a); setResult(null); setNote(a.notas || ''); setScreen('detail'); }}
                style={{ background: i === 0 ? '#0f172a' : '#020617', border: '1px solid ' + (a.resultado_json?.urgente ? '#ef444430' : '#1e293b'), borderRadius: '8px', padding: '12px', marginBottom: '8px', cursor: 'pointer' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                  <span style={{ fontSize: '11px', color: '#38bdf8', fontWeight: i === 0 ? 700 : 400 }}>{fmt(a.created_at)}</span>
                ...
                {a.notas && <div style={{ marginTop: '5px', padding: '4px 8px', background: '#f59e0b10', border: '1px solid #f59e0b20', borderRadius: '4px', fontSize: '10px', color: '#fcd34d' }}>📝 {a.notas.slice(0, 80)}</div>}
              </div>
            ))}
          </div>
        )}
        ...
        {screen === 'new' && sel && (
          <div className="fin" style={{ padding: '16px' }}>
            ...
            <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: '9px', padding: '14px', marginBottom: '10px' }}>
              ...
              <textarea value={caseText} onChange={e => setCaseText(e.target.value)} placeholder="Digite, cole ou use o microfone..." className="inp" style={{ minHeight: '100px', resize: 'vertical', lineHeight: '1.6', padding: '9px' }} />
        ...
        {screen === 'dashboard' && sel && (
          <div className="fin" style={{ padding: '16px' }}>
            ...
            {patA.length === 0 ? <div style={{ textAlign: 'center', padding: '32px', color: '#475569', fontSize: '12px' }}>Nenhuma analise ainda.</div> : (() => {
              const lat = patA[0]?.resultado_json || {};
              const sv = lat.paciente?.sinais_vitais || {};
              const qpts = lat.sepse?.qsofa?.pontos ?? 0;
              const ic = lat.sepse?.indice_choque?.valor || '';
              const icn = parseFloat(ic) || 0;
              const flags = (lat.red_flags || []).slice(0, 4);
              const hip = (lat.hipoteses || [])[0];
              const passo = (lat.passos || [])[0];
              const sc = qpts >= 2 || icn >= 1 || lat.urgente;
              const scolor = sc ? '#ef4444' : qpts >= 1 ? '#f59e0b' : '#22c55e';
              const slabel = sc ? 'ATENCAO IMEDIATA' : qpts >= 1 ? 'MONITORAR' : 'ESTAVEL';
              ...
              return (
                <div>
                  ...
                  {flags.length > 0 && <div style={{ background: '#0f172a', border: '1px solid #ef444430', borderLeft: '3px solid #ef4444', borderRadius: '10px', padding: '12px', marginBottom: '12px' }}>
                    <div style={{ fontSize: '9px', color: '#ef4444', fontWeight: 700, letterSpacing: '0.1em', marginBottom: '8px' }}>RED FLAGS</div>
                    {flags.map((f, i) => <div key={i} style={{ display: 'flex', gap: '7px', padding: '4px 0' }}><span style={{ color: '#ef4444' }}>!</span><span style={{ color: '#fca5a5', fontSize: '11px' }}>{f}</span></div>)}
                  </div>}
        ...
        {screen === 'evolution' && sel && (
          <div className="fin" style={{ padding: '16px' }}>
            ...
            {patA.slice().reverse().map((a, i, arr) => {
              ...
              return (
                <div key={a.id} style={{ display: 'flex', gap: '12px', marginBottom: '8px' }}>
                  ...
                  <div style={{ flex: 1, background: '#020617', border: '1px solid ' + (a.resultado_json?.urgente ? '#ef444430' : '#1e293b'), borderRadius: '8px', padding: '10px 12px', cursor: 'pointer' }} onClick={() => { setSelA(a); setNote(a.notas || ''); setScreen('detail'); }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
